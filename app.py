...
def indexar_armaduras(ifc_file, ifc_path: str | None = None) -> dict:
    ...
    def _bar_sapata(barra):
        ...
        # Extrair diâmetro do fio via raio do SweptDiskSolid
        # Usar lista para permitir atribuição dentro de função aninhada
        _diam = [1.0]  # padrão Ø10mm = 1cm
        _vis2 = set()
        def _find_sds(pid, d=0):
            ...
            if _et2 == "IFCSWEPTDISKSOLID":
                _p2 = _ed2.split(",")
                try: _diam[0] = abs(float(_p2[1].strip())) * 2.0
                except: pass
                return
            for _r in _re.findall(r"#(\d+)", _ed2): _find_sds(_r, d+1)
        _find_sds(repr_id)
        comp_total = corpo + _diam[0]  # Box Length = corpo + diâmetroiâmetro
...
def processar_ifc(caminho: str, nome_projeto: str, id_projeto: str) -> list[dict]:
    ...
    try:
        import ifcopenshell.util.element as _iue_sap
    except Exception:
        _iue_sap = None
    ...
    for _elem in ifc.by_type("IfcFooting"):
        ...
        try:
            if _iue_sap:
                _ps2 = _iue_sap.get_psets(_elem)
                _tem_geo = bool(
                    _ps2.get("TQS_Geometria") or _ps2.get("Pset_TQS_Geometria")
                )
        except Exception:
            pass
        _key = (_nome, _pav)
...
def gerar_pdf(registros: list[dict], nome_projeto: str) -> io.BytesIO:
    ...
    for reg in registros:
        ...
        c.drawCentredString(_cx_qr, y + 2.5*mm, f"Gerado em: {_hoje}")
